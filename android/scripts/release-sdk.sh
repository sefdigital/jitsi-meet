...
# Now build and publish the Julian Schrömges Beratung SDK and its dependencies
echo "Building and publishing the Julian Schrömges Beratung SDK"
pushd ${THIS_DIR}/../
./gradlew clean 
...
if [[ $DO_GIT_TAG == 1 ]]; then
    ...
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Julian Schrömges Beratung SDK + dependencies: ${SDK_VERSION}"
    popd
