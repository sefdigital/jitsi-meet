...
echo "Releasing Investmentberatung SJ SDK ${SDK_VERSION}"
...
# Now build and publish the Investmentberatung SJ SDK and its dependencies
echo "Building and publishing the Investmentberatung SJ SDK"
pushd ${THIS_DIR}/../
./gradlew clean 
...
if [[ $DO_GIT_TAG == 1 ]]; then
    ...
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Investmentberatung SJ SDK + dependencies: ${SDK_VERSION}"
    popd
