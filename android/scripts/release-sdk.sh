...
echo "Releasing Investmentberatung JS SDK ${SDK_VERSION}"
...
# Now build and publish the Investmentberatung JS SDK and its dependencies
echo "Building and publishing the Investmentberatung JS SDK"
pushd ${THIS_DIR}/../
./gradlew clean 
...
if [[ $DO_GIT_TAG == 1 ]]; then
    ...
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Investmentberatung JS SDK + dependencies: ${SDK_VERSION}"
    popd
